...
*   **编译与运行测试**: 每次修改完代码后，必须通过本地终端运行 `$env:JAVA_HOME="C:\Program Files\Java\jdk-25.0.3"; .\gradlew.bat jar` 进行编译校验，确保没有语法和编译期报错。
*   **日志规范**: 绝对禁止使用 `System.out` 或 `System.err` 输出调试或运行日志。在进行日志记录时，必须使用 Log4j 2 Logger，且必须只使用 `com.pppopipupu.decaylib.DecayLib.LOG`。
*   **操作与读写限制**: 你必须只读取 `C:\Modding\NTMC`、`C:\Users\pppop\Desktop\decaylib` 和 `C:\Users\pppop\Desktop\HBMAletheiaAddon` 内的文件，只能修改 `C:\Users\pppop\Desktop\decaylib` 和 `C:\Users\pppop\Desktop\HBMAletheiaAddon` 内的文件，绝对不可读取或修改任何其他文件。
